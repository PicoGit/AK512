...
#define PROJECT_1_3_5_DSP_MATRIX	  ( 0x135u )
#define PROJECT_1_3_6_DSP_HILBERT	  ( 0x136u )
...
#define PROJECT_2_4_5_AD_SINGLE_REP		( 0x245u )
...
#define PROJECT_4_1_5_CRC				( 0x415u ) 
#define PROJECT_4_1_6_BIST			    ( 0x416u ) 
#define PROJECT_4_1_7_ECC			    ( 0x417u ) 

#define PROJECT_TYPE  PROJECT_1_3_3_DSP_ACR
...
int main(int argc, char** argv) 
{
  ...
  /*-----------------------------------------------------------------------*/
    ...
    /*-----------------------------------------------------------------------*/
		...
		for (int i=0;i < 128; i++)
		{
			t =  1.0f / 16000 * (float)i;
			v = sin(2 * 3.141592 * 1000 * t) * 0.25
					;

			fr_InputData[i] = Float2Fract(v) ;
...
void Calc_FFT(void)
{
	...
	/*------------------------------------------------------------------------*/
		for (i = 0; i < FFT_POINT;i++)
		{
			...
			//frc_FftData[i].real = fr_InputData[i];
...
#elif (PROJECT_TYPE == PROJECT_1_3_5_DSP_MATRIX )
#define ROWS 10
#define COLS 5

volatile fractional src1[ROWS][COLS];
volatile fractional src2[ROWS][COLS];
volatile fractional Adddst[ROWS][COLS];
volatile fractional Subdst[ROWS][COLS];
volatile fractional MMdst[ROWS][COLS];
volatile fractional Scldst[ROWS][COLS];
volatile fractional Trsdst[ROWS][COLS];
volatile fractional Invdst[ROWS][COLS];
volatile int pivotFlag[COLS];
volatile int swappedRows[COLS];
volatile int swappedCols[COLS];
...
int main(int argc, char** argv) 
{
	...
	for (int y=0 ; y<ROWS;y++)
	{
		...
		{
			src1[y][x] = Q31(0.01 * x + 0.02 * y + 0.3);
			src2[y][x] = Q31(0.025 * x + 0.034 * y + 0.3);
		}
	}
		
		
	p = MatrixAdd(ROWS,COLS,Adddst,src2,src1);
    p = MatrixSubtract(ROWS,COLS,Subdst,src2,src1);
	p = MatrixScale(ROWS,COLS,Scldst,src1,Q31(0.25));
	p = MatrixTranspose(COLS,COLS,Trsdst,src1);
	p = MatrixMultiply(COLS,COLS,COLS,MMdst,src2,src1);
	p = MatrixInvert(COLS,Invdst,src2,pivotFlag,swappedRows,swappedCols);
	while(1)
	{

	}
}
#elif (PROJECT_TYPE == PROJECT_1_3_6_DSP_HILBERT)
...
#elif (PROJECT_TYPE == PROJECT_4_1_5_CRC)
/*----------------------------------------------------------------------------*/
/* <Chapter>        CHAPTER_2_B_1_CRC */
/* @brief       CRCテスト */
/* <Peripharal>     CRC */
/*----------------------------------------------------------------------------*/
/*----------------------------------------------------------------------------*/
...
#define REVERSE_BIT_ALG_MCP_DSP 1 
#define REVERSE_BIT_ALG_EDWINFREED 2 

#define REVERSE_BIT_ALG REVERSE_BIT_ALG_EDWINFREED
/*----------------------------------------------------------------------------*/
/*変数定義*/
/*----------------------------------------------------------------------------*/
unsigned char message_8bit[] = {0x44,0x43,0x42,0x41};   //送信メッセージ
unsigned short message_16bit[] = {0x4443,0x4241};   //送信メッセージ
unsigned int message_32bit[] = {0x44434241};   //送信メッセージ
...
volatile int CRC_Result[77];

const unsigned int CRC_Parameter[50][7] =
{
    //Poly,Init,NonDirect,RefIn,RefOut,XorOut,PolyORDER
    ...
    {0x1021,0xC6C6,0x606A,0x0001,0x0001,0x0000,0x10},//CRC_16_A
	...
	{0x814141AB	,0x00000000	,0,	0,	0,0x00000000,0x20},//CRC-32_AIXM
	{0xF4ACFB13	,0xFFFFFFFF	,0,	1,	1,0xFFFFFFFF,0x20},//CRC-32_AUTOSAR
	{0xA833982B	,0xFFFFFFFF	,0,	1,	1,0xFFFFFFFF,0x20},//CRC-32_BASE91-D
	{0x04C11DB7	,0xFFFFFFFF	,0,	0,	0,0xFFFFFFFF,0x20},//CRC-32_BZIP2
	{0x8001801B	,0x00000000	,0,	1,	1,0x00000000,0x20},//CRC-32_CD_ROM_EDC
	{0x04C11DB7	,0x00000000	,0,	0,	0,0xFFFFFFFF,0x20},//CRC-32_CKSUM
	{0x1EDC6F41	,0xFFFFFFFF	,0,	1,	1,0xFFFFFFFF,0x20},//CRC-32_ISCSI
	{0x04C11DB7	,0xFFFFFFFF	,0,	1,	1,0xFFFFFFFF,0x20},//CRC-32_ISO-HDLC
	{0x04C11DB7	,0xFFFFFFFF	,0,	1,	1,0x00000000,0x20},//CRC-32_JAMCRC
	{0x741B8CD7	,0xFFFFFFFF	,0,	1,	1,0x00000000,0x20},//CRC-32_MEF
	{0x04C11DB7	,0xFFFFFFFF	,0,	0,	0,0x00000000,0x20},//CRC-32_MPEG-2
	{0x000000AF	,0x00000000	,0,	0,	0,0x00000000,0x20}//CRC-32_XFER
...
static unsigned long CRC_Calc16(unsigned short *message,unsigned int Length,unsigned char Algo);
...
int main(void)
{
    ...
    /*------------------------------------------------------------------------*/
        while (1)
        {
			...
			CRC_Result[50] = crc16_CPU(0,message_8bit,DataLength);
			
            DataLength = sizeof(message_16bit)/sizeof(unsigned short);
            CRC_Result[13] = CRC_Calc16(message_16bit , DataLength, CRC_16);
            CRC_Result[14] = CRC_Calc16(message_16bit , DataLength, CRC_16_CCITT_FALSE);  
            CRC_Result[15] = CRC_Calc16(message_16bit , DataLength, CRC_16_ARC);  
            CRC_Result[16] = CRC_Calc16(message_16bit , DataLength, CRC_16_AUG_CCITT);  
            CRC_Result[17] = CRC_Calc16(message_16bit , DataLength, CRC_16_CDMA2000);  
            CRC_Result[18] = CRC_Calc16(message_16bit , DataLength, CRC_16_DDS_110);  
            CRC_Result[19] = CRC_Calc16(message_16bit , DataLength, CRC_16_DECT_R);  
            CRC_Result[20] = CRC_Calc16(message_16bit , DataLength, CRC_16_DECT_X);  
            CRC_Result[21] = CRC_Calc16(message_16bit , DataLength, CRC_16_DNP);
            CRC_Result[22] = CRC_Calc16(message_16bit , DataLength, CRC_16_EN_13757);  
            CRC_Result[23] = CRC_Calc16(message_16bit , DataLength, CRC_16_GENIBUS);  
            CRC_Result[24] = CRC_Calc16(message_16bit , DataLength, CRC_16_MAXIM);
            CRC_Result[25] = CRC_Calc16(message_16bit , DataLength, CRC_16_MCRF4XX); 
            CRC_Result[26] = CRC_Calc16(message_16bit , DataLength, CRC_16_RIELLO); 
            CRC_Result[27] = CRC_Calc16(message_16bit , DataLength, CRC_16_T10_DIF);  
            CRC_Result[28] = CRC_Calc16(message_16bit , DataLength, CRC_16_TELEDISK);  
            CRC_Result[29] = CRC_Calc16(message_16bit , DataLength, CRC_16_TMS37157);
            CRC_Result[30] = CRC_Calc16(message_16bit , DataLength, CRC_16_USB);
            CRC_Result[31] = CRC_Calc16(message_16bit , DataLength, CRC_16_A);
            CRC_Result[32] = CRC_Calc16(message_16bit , DataLength, CRC_16_KERMIT);
            CRC_Result[33] = CRC_Calc16(message_16bit , DataLength, CRC_16_MODBUS);
            CRC_Result[34] = CRC_Calc16(message_16bit , DataLength, CRC_16_X_25);
            CRC_Result[35] = CRC_Calc16(message_16bit , DataLength, CRC_16_XMODEM);  
		    CRC_Result[36] = CRC_Calc16(message_16bit , DataLength, CRC_16_USB);
            CRC_Result[37] = CRC_Calc16(message_16bit , DataLength, CRC_INHERIT);
            CRC_Result[38] = CRC_Calc16(message_16bit , DataLength, CRC_INHERIT);
			CRC_Result[51] = crc16_CPU_Table(0,message_8bit,DataLength);
			
			
            DataLength = sizeof(message_32bit)/sizeof(unsigned int);
...
/*------------------------------------------------------------------------*/
/* @fn        ReverseBitOrder(unsigned int data) */
...
#if (REVERSE_BIT_ALG ==  REVERSE_BIT_ALG_MCP_DSP)
        Result[0].real = (unsigned int)data;
        Result[0].imag = 0;
        BitReverseComplex(0,&Result[0]);
        Nop();
    //    result = Dresult->real;
#elif (REVERSE_BIT_ALG ==  REVERSE_BIT_ALG_EDWINFREED)
        //奇数ビットと偶数ビットの交換
...
static void CRC_Set_Parameter(unsigned char Algo,unsigned char DataWidth)
{
    ...
    /*------------------------------------------------------------------------*/
		CRCCONbits.ON = 1u;
		CRCCONbits.LENDIAN = Input_Reflected;
		CRCCONbits.DWIDTH = DataWidth - 1u;
		CRCCONbits.PLEN = PolyNomialOrder - 1u;
        CRCXOR = PolyNomial;
        CRCWDAT = NonDirectValue;

}
/*------------------------------------------------------------------------*/
/* @fn            CRC_Flush_Clean_Output(void)*/
//* @brief         CLCのフラッシュ、クリーン、出力*/
/* @param[in]     無し*/
/* @param[out]    CRC計算結果*/
/*------------------------------------------------------------------------*/
static unsigned long CRC_Flush_Clean_Output(void)
{
    ...
    /*------------------------------------------------------------------------*/
        ...
        CRCCONbits.DWIDTH = PolyNomialOrder - 1;
        if (PolyNomialOrder == CRC_POLY_ORDER_16BIT){CRCDAT = 0u;}
        else if(PolyNomialOrder == CRC_POLY_ORDER_8BIT){*((uint8_t *)&CRCDAT) = 0x00u;}
        
        CRCCONbits.CRCGO = 1u;
    /*------------------------------------------------------------------------*/
    /* データクリーンナップ*/
    /*------------------------------------------------------------------------*/
        while(IFS4bits.CRCIF == 0u){;}
        CRCCONbits.DWIDTH = DataWidth - 1;
        CRCCONbits.CRCGO = 0u;
        IFS4bits.CRCIF = 0u;
    ...
    /*------------------------------------------------------------------------*/
        if (Output_Reflected == 0u)
        {
            ...
            {
                return((uint32_t)CRCWDAT);
            }
            else
            {
                return(((uint32_t)CRCWDAT ^ FinalXorValue) );
            }
        }
        else  
        {
            if (FinalXorValue == 0x00u)
            {
                CRC_Result = ReverseBitOrder(CRCWDAT );
...
/*------------------------------------------------------------------------*/
/* @fn            unsigned long CRC_Calc8(unsigned char *message,unsigned int Length,unsigned char Algo)*/
/* @brief         8bitデータのCRCモジュール計算*/
/* @param[in]     message:計算対象バイト列*/
/* @param[in]     Length:計算対象バイト長*/
/* @param[in]     Algo:CRCタイプ*/
/* @param[out]    CRC計算結果*/
/*------------------------------------------------------------------------*/
static unsigned long CRC_Calc8(unsigned char *message,unsigned int Length,unsigned char Algo)
{
...
/*------------------------------------------------------------------------*/
/* @fn        unsigned long CRC_Calc16(unsigned short *message,unsigned int Length,unsigned char Algo)*/
/* @brief         16ビットデータのCRCモジュール計算*/
/* @param[in]     message:計算対象バイト列*/
/* @param[in]     Length:計算対象バイト長*/
/* @param[in]     Algo:CRCタイプ*/
/* @param[out]    CRC計算結果*/
...
#elif (PROJECT_TYPE == PROJECT_4_1_6_BIST)
/*----------------------------------------------------------------------------*/
/* <Chapter>        PROJECT_4_1_6_MBIST */
/* @brief       CRCテスト */
/* <Peripharal>     CRC */
/*----------------------------------------------------------------------------*/
/*----------------------------------------------------------------------------*/
...
int main(void)
{
	...
	if (RCONbits.SWR == 1)
	{
		if (MBISTCONbits.MBISTDONE == 1)
		{
			if (MBISTCONbits.MBISTSTAT == 1)
			{
				if (MBISTCONbits.FLTINJ == 1)
				{
					//MBIST 合格(フォルト注入)
					
					MBISTCONbits.FLTINJ = 0;
					MBISTCONbits.MBISTEN = 1;
...
#elif (PROJECT_TYPE == PROJECT_4_1_7_ECC)
/*----------------------------------------------------------------------------*/
/* <Chapter>        PROJECT_4_1_7_ECC */
/* @brief       ECCテスト */
/* <Peripharal>     CRC */
/*----------------------------------------------------------------------------*/
/*----------------------------------------------------------------------------*/
...
int XMem[10] __attribute__ ((space(xmemory), near));
int Result;
/*----------------------------------------------------------------------------*/
/* Main関数 */
/*----------------------------------------------------------------------------*/
int main(void)
{
	...
	RAMXECCCONbits.FLTINJ = 0;
	
			
	RAMXECCFADDR = & XMem[0];
	RAMXECCFPTRbits.FLT1LPTR = 2;
	RAMXECCFPTRbits.FLT2LPTR = 0x3F;
	...
	PWBXECCCONbits.FLTINJ = 0;
	PWBXECCFPTRbits.FLT1LPTR = 2;
	PWBXECCFPTRbits.FLT2LPTR = 0x3F;
